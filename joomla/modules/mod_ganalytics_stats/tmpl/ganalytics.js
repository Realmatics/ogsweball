jQuery(document).ready(function() {
	...
	jQuery('input[name=mod_date_from]').change(function() {
		jQuery(this).closest('.mod-ganalytics-table').parent().find('.mod-ga-chart').gaChart('refresh', {
			start: jQuery(this).val()
		});
	});
	jQuery('input[name=mod_date_to]').change(function() {
		jQuery(this).closest('.mod-ganalytics-table').parent().find('.mod-ga-chart').gaChart('refresh', {
			end: jQuery(this).val()
		});
	});
	...
	jQuery('.date-range-button').click(function(){
		...
		jQuery(this).parent().parent().find('.mod-ga-chart').gaChart('refresh', {
			dateRange: range
		});
	});
});
...
function showModCharts(){
	jQuery('.mod-ga-chart').gaChart('refresh', {
		start: jQuery('input[name=mod_date_from]').val(),
		end: jQuery('input[name=mod_date_to]').val()
	});
}