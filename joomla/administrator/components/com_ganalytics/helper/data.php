...
require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_ganalytics'.DS.'libraries'.DS.'gapc'.DS.'contrib'.DS.'apiAnalyticsService.php');
		...
		try {
			...
			foreach ($service->management_accounts->listManagementAccounts()->getItems() as $account) {
				foreach ($service->management_webproperties->listManagementWebproperties($account->getId())->getItems() as $webItem) {
					foreach ($service->management_profiles->listManagementProfiles($account->getId(), $webItem->getId())->getItems() as $profile) {
				...
				foreach($data->getRows() as $item) {
					...
					foreach($item as $index => $value) {
						...
						if(stripos($header->name, 'ga:date') !== false) {
							...
							$formatted = '<a href="http://'.$url.'" target="_blank">'.$value.'</a>';
							...
							$formatted = '<a href="'.JRoute::_('index.php?option=com_ganalytics&view=page&tmpl=component&gaid='.$profile->id.'&path='.base64_encode($value)).'">'.$value.'</a>';
						} else {
