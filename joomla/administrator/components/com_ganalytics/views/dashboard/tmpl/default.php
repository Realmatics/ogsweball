...
<td style="padding-right:20px">
	...
	echo JHtml::_('calendar', strftime('%Y-%m-%d', strtotime('-1 month')), 'date_from', 'date_from', '%Y-%m-%d', array('size' => 10));
	?>
</td>
<td style="padding-right:20px">
...
<?php }?>
<li><a href="#" id="add-tab-button"><span class='ui-icon ui-icon-plusthick'></span></a></li>
	</ul>
<?php
...
<?php foreach ($this->statsViews as $view) {
		...
		<?php
		$scriptCode .= "jQuery('#widgetForm-".$view->id."').gaChart({
			chartDivID: 'gaChartDiv".$view->id."',
			gaid: jQuery('#profiles').val(),
			url: 'index.php?option=com_ganalytics&view=dashboard&format=raw&layout=data&id=".$view->id."',
			start: jQuery('#date_from').val(),
			end: jQuery('#date_to').val(),
