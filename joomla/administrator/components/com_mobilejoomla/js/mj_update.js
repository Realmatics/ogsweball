...
window.addEvent('domready',function(){
	function checkUpdate(){
		if(typeof Request == "function"){
			new Request.HTML( {
				url: 'http://www.mobilejoomla.com/getver.php?v=' + escape('1.2.2'),
				method: 'get',
				update: 'mjlatestver',
				onSuccess : function(tree, elements, response){
					if(version_compare('1.2.2', response)>0){
						$('mjlatestverurl').setStyle('display', 'block');
					}
				}
			}).send();
		} else if(typeof Ajax == "function"){
			new Ajax( 'http://www.mobilejoomla.com/getver.php?v=' + escape('1.2.2'), {
				method: 'get',
				update: $('mjlatestver'),
