...
class plgContentjsmallfib extends JPlugin
{
	...
	function onContentPrepare($context, &$article, &$params, $limitstart = null) {
		...
		//if($article->text!=str_replace("xdlx","",$article->text)){$access_rights = 2;}
		if($article->text!=str_replace("x-onlydownload-x","",$article->text)){$access_rights = 2;}
		...
		if ($repository)
		{
			if($article->text!=str_replace("x-onlydownload-x","",$article->text)){$starting_dir = downloads;}else{$starting_dir = images;}
			$starting_dir = $starting_dir.DS.$repository;
			//$starting_dir = $this->default_absolute_path.DS.$repository;
			//$starting_dir = images;
			
			if($article->text!=str_replace("x-dir:","",$article->text))
			{
				$jscontent=$article->text;
				$jspos=0;
				$jsarrayser='x-dir:';
				$jsarrayser1='}';
				$jsposa = stripos($jscontent, $jsarrayser, $jspos);
				$jsposb = stripos($jscontent, $jsarrayser1, $jsposa);
				$jsposa = $jsposa + 6;
				$jscontentgefundenl = $jsposb - $jsposa;
				$jscontentgefunden = substr($jscontent, $jsposa, $jscontentgefundenl);			
				$starting_dir = $jscontentgefunden;}
...
//		if(!stristr(str_replace("/", "\\", $dir), str_replace("/", "\\", $_SERVER["DOCUMENT_ROOT"]))) 
		...
		if($article->text!=str_replace("x-folders-x","",$article->text)){
		// Folders first
		if ($dirs)
		{
			foreach ($dirs as $a_dir)
			{
				...
				else
				{
					$text .= "<tr class='row $row_style' onmouseover='this.className=\"highlighted\"' onmouseout='this.className=\"row $row_style\"'>"
						...
						."	<a href='".$this->baselink."&dir=".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a_dir)."'>"
                                                            ."<img src=\"".$this->imgdirNavigation."folder.png\" width='".$this->icon_width."' align='left' style='margin:0; padding:0px 0px 0px 35px;'/></a>"
						."	</td>"
						."	<td class='fileName'>"
				...
				if($article->text!=str_replace("x-subobjects-x","",$article->text)){
				//$incli = 'jsmallfib_sub.php?vertiz='.'$a_dir';	
				$incli = "jsmallfib_sub.php";	
				include ($incli);								
				//include 
				$vertiz++;
				}
		...
		if($files)
		{
			foreach ($files as $a_file)
			{
				...
				else
				{
					...
					$file_icon_image	= "<img src=\"".$this->fileIcon($a_file["extension"])."\" width='".$this->icon_width."' align='left' border='0' style='margin:0; padding:0px 0px 0px 35px;' />";
					$file_icon_td_end	= "</td>";
				}
				...
				else
				{
					...
					else
					{
						...
						{
							$file_link_a_tag_begin = "<a href='".$this->baselink."&dir=".$this->urlEncodePreserveForwardSlashes($masked_dir)."&download_file=".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a_file["name"])."' onclick=\"_gaq.push(['_trackPageview', '/".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a_file["name"])."']);\">";
						}
