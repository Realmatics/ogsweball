...
if($open_dir = @opendir(html_entity_decode(str_replace("\\", "/", $dir."/"))))
		{
			//$dirssub = array();
			//$filessub = array();
			$dirssub[$vertiz][$vertiz1][$vertiz2][$vertiz3] = array();
			$filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3] = array();
			$i = 0;
				...
				if($it != "." && $it != "..")
				{
					if(is_dir($dir.DS.$it))
					{
						if(!in_array($it, $hidden_folders))
							$dirssub[$vertiz][$vertiz1][$vertiz2][$vertiz3][] = htmlspecialchars($it);
					}
					...
					else if(!in_array($it, $hidden_files))
					{
						...
						if (!$matched_prefix && !$matched_extension && (!$file_filter_pattern_required || $file_filter_pattern_matched))
						{
							...
							$filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3][$i]["name"]	= htmlspecialchars($it);
							$it			= $dir."/".$it;
							$filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3][$i]["extension"]	= $this->fileExtension($it);
							$filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3][$i]["size"]	= $this->fileRealSize($it);
							$filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3][$i]["changed"]	= filemtime($it);							
							$i++;
						}
					}
				}
...
// Subfolders first
		if ($dirssub[$vertiz][$vertiz1][$vertiz2][$vertiz3])
		{
			foreach ($dirssub[$vertiz][$vertiz1][$vertiz2][$vertiz3] as $a_dir)
			{
				...
				else
				{
					$text .= "<tr class='row $row_style' onmouseover='this.className=\"highlighted\"' onmouseout='this.className=\"row $row_style\"'>"
						."	<td width='190px'><img src=\"".$this->imgdirNavigation."subs.png\" width='18' style='margin:0; padding:0px 0px 6px 130px;'/>"
						."	<a href='".$this->baselink."&dir=".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a1_dir."/".$a2_dir."/".$a3_dir."/".$a4_dir."/".$a_dir)."'>"
                                                            ."<img src=\"".$this->imgdirNavigation."folder.png\" width='".$this->icon_width."' /></a>"
						."	</td>"
						."	<td class='fileName'>"
						."	<a href='".$this->baselink."&dir=".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a1_dir."/".$a2_dir."/".$a3_dir."/".$a4_dir."/".$a_dir)."'>".$this->chosen_encoding($a_dir)."</a>"
		...
		// Now the subfiles
		if($filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3])
		{
			foreach ($filessub[$vertiz][$vertiz1][$vertiz2][$vertiz3] as $a_file)
			{
			if($a_file["name"]){
				$row_style = ($row ? "odd" : "even");
				...
				{
					$file_icon_td_begin	= "<td width='190px'><img src=\"".$this->imgdirNavigation."subs.png\" width='18' style='margin:0; padding:0px 0px 1px 130px;'/>";
				...
				else
				{
					...
					else
					{
						...
						{
							$file_link_a_tag_begin = "<a href='".$this->baselink."&dir=".$this->urlEncodePreserveForwardSlashes($masked_dir)."&download_file=".$this->urlEncodePreserveForwardSlashes($masked_dir."/".$a1_dir."/".$a2_dir."/".$a3_dir."/".$a4_dir."/".$a_file["name"])."'>";
							//$file_link_a_tag_begin = "<a href=\"".$this->makeForwardSlashes($this->chosen_encoding($relative_dir.DS.$a1_dir."/".$a2_dir."/".$a3_dir."/".$a4_dir."/".$a_file["name"]))."\" ".($is_direct_link_to_files > 1 ? "target='_blank'" : "").">";
						}
