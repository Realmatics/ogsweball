...
class plgContentVersion extends JPlugin
{
	...
	public function onContentAfterSave($context, &$article, $isNew)
	{
		...
		// start check versioning on/off
		//if($article->introtext!=str_replace("x-version-x","",$article->introtext)){
		//$versioning		= JRequest::getVar('jform[versioning]');
		//$versioning = $value->publish_up."test";
		//$versioning = JRequest::getInt('jform[versioning]')."test";
		//$input = JFactory::getApplication() -> input;
		//$versioning = $input -> post -> get('jform[versioning]','','STRING')."test";
		//$versioning = $params['versioning']."test";
		if($article->versioning=="1"){
		...
		//if($doc->introtext!=str_replace("x-version-x","",$doc->introtext)){$doc->checked_out = $versioning;}
		//$doc->metakey = $article->versioning;
		...
		//$con = mysql_connect("localhost","web161","4RNlW0UP");

		//mysql_select_db("usr_web161_1") or die ("Die Datenbank existiert nicht");
		
		//mysql_query("UPDATE `usr_web161_1`.`jo254_content` SET `versioning` = '0' WHERE `id` = '.$article -> id.'");
		//mysql_query("UPDATE `usr_web161_1`.`jo254_content` SET `versioning` = '0' WHERE `id` = '86'");
